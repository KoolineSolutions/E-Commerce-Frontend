        ...
        <>
            ...
            <nav className="flex items-center justify-between bg-gray-800 p-4">
                ...
                <div className={`flex items-center space-x-5 gap-2 transition-all duration-700 ${showNav ? 'block' : 'hidden'}`}>
                    {
                        currentUser && currentUser != undefined && <button onClick={() => setShowCart(!showCart)} className="text-white cursor-pointer text-md flex gap-1 flex-col items-center"><FaShoppingCart title="Cart" className="text-xl" /></button>
                    }
                    {
